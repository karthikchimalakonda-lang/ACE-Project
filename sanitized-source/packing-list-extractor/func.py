...
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# #  LOGGING  â€” writes to both OCI logging AND stdout (visible in Code Editor)
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# #  FOLDER HELPERS  â€” all paths rooted at Packing List/{safe_req}/
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# DETECT THE FORMAT FIRST â€” this document may be one of three types:

# FORMAT A â€” FLAT LIST (no pkg_no, no hierarchy):
#   Columns: Item No | Description | Quantity | Weight | Dimensions
#   Example: "1  Sample Product 1  4  3.0  2x3x4 cm"
#   Rule: Every row is an independent item. No sub-items. No nesting.
#   Extract each row as a separate package entry with sub_items: []

# FORMAT B â€” CONTAINER/PALLET/ITEM (OTM structured):
#   Columns: Item Code | Description | HS Code | Container | Pallet | Pkg No | Qty | UOM
#   Example: "Item-1  Industrial Product 1  HS1001  C1  P1  1 OF 6  6  PCS"
#   Rule: Every row is independent. Container and Pallet are columns on every row.
#   pkg_no may appear on some rows only â€” carry forward to rows without it.
#   Extract each row as a separate package entry. Use container_no and pallet_no fields.
#   sub_items: [] for all rows.

# FORMAT C â€” BUNDLE/SUB-ITEM (hierarchical):
#   Columns: Pkg No | Packing Style | Item No | Description | Qty | Net Wt | Gross Wt | Dim | CBM
#   Example main row:  "1 OF 5  Bundle  1320-2  Steel Structure  1  496.19  10900  1170x190x225  50.018"
#   Example sub row:   "        1320-4  Steel Structure  1  496.19" (no gross_weight, no dim, no cbm)
#   Rule: Main row has pkg_no + gross_weight_kg + dimensions_cm + cbm.
#         Sub-rows have item_no + quantity + net_weight ONLY â€” no gross/dim/cbm.
#         Sub-rows belong to the LAST main row above them.
#         Nest sub-rows inside parent package as sub_items array.

# HOW TO DETECT:
#   - Has "Container" and "Pallet" columns â†’ FORMAT B
#   - Has "Pkg No" + "Packing Style" + rows with gross_weight AND rows without â†’ FORMAT C
#   - Simple numbered rows, no container/pallet/pkg columns â†’ FORMAT A
...
# CRITICAL RULES:
# - FORMAT A: pkg_no = item number (1, 2, 3...), sub_items = [], container_no = null
# - FORMAT B: pkg_no = carry-forward "X OF Y" if present else null,
#   container_no from Container column, pallet_no from Pallet column, sub_items = []
# - FORMAT C: pkg_no must NEVER be null â€” carry forward from context.
#   Sub-rows (no gross_weight, no dimensions, no cbm) â†’ nest in sub_items of parent.
#   Do NOT create top-level package entries for sub-rows in FORMAT C.
# - SKIP header rows: column headers, "SUMMARY PACKING LIST", "Shipment No.", "Vendor:"
# - TOTAL row at bottom â†’ extract to total fields only, NOT packages array
# - Extract EVERY data row â€” do not skip any
...
# # STRUCTURE RULES:
# # - "packages" is a FLAT list of every individual PACKAGE (identified by pkg_no like "1 OF 5")
# # - pkg_no is the PACKAGE NUMBER like "1 OF 5", "2 OF 5" â€” NOT the item number
# # - item_no is the PRIMARY ITEM NUMBER for the package (e.g. "811A7110-02")
# # - gross_weight_kg, dimensions_cm, cbm: from the MAIN package row only
# # - SUB-ITEMS / ACCESSORIES (e.g. "Transportation Saddle", saddles, brackets, accessories):
# #     - These are rows with NO pkg_no, NO quantity, NO gross_weight, NO dimensions
# #     - They belong to the PARENT package above them
# #     - Collect them into a "sub_items" array nested inside the parent package object
# #     - Do NOT create a separate top-level package entry for them
# # - If this chunk contains no packing list rows, return empty packages []

# # RANGE-BASED PACKING LISTS (e.g. "Package #1-2", "Package #10-16"):
# # - Set pkg_no to the range string e.g. "1-2", "10-16"
# # - Set quantity to total count of packages in that range
# # - gross_weight shown is TOTAL for the range â€” do NOT divide it
# # - Each range = exactly ONE row in packages array

# # HEADER TOTALS:
# # - Some documents show total gross weight in the document header
# # - Extract this as total_gross_weight_kg even if no TOTAL row exists
...
# # CRITICAL:
# # - Extract EVERY row â€” do not skip any
# # - pkg_no must NEVER be null â€” use the carry-forward value from context if needed
# # - SUMMARY/HEADER ROWS: pkg_no is a range like "1-5" AND unit is "PKG" AND
# #   gross_weight_kg is very large â†’ extract to total fields, NOT packages array
...
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# #  SECTION 5  â€”  JSON PARSING & REPAIR
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
#         return ""
#     return (
#         f"CONTINUATION CONTEXT: The previous chunk ended at package '{last_pkg_no}'.\n"
#         f"If this chunk starts with rows that have NO gross_weight_kg, NO dimensions_cm, "
#         f"NO cbm â€” these are FORMAT C sub-items still belonging to '{last_pkg_no}'.\n"
#         f"Nest them in sub_items under '{last_pkg_no}'.\n"
#         f"Only start a new pkg_no when you see an explicit 'X OF Y' line "
#         f"(e.g. '3 OF 5') OR a new container/pallet assignment in FORMAT B.\n"
#     )
# # def _make_continuation_context(last_pkg_no: Optional[str]) -> str:
# #     if not last_pkg_no:
# #         return ""
...
#         if re.search(r"\d[\d,]*\.\d+\s+KG", line, re.IGNORECASE)
#     ]
#     if len(data_line_indices) <= max_rows:
#         return [page_text]
#     log.info("  Dense page: %d data rows â€” splitting into sub-chunks of â‰¤%d",
#              len(data_line_indices), max_rows)
...
#         batch   = body_lines[i:i + max_rows]
#         is_last = (i + max_rows) >= len(body_lines)
#         text    = header + "\n" + "\n".join(batch)
#         if is_last:
#             text += "\n" + footer
#         sub_chunks.append(text)
#     log.info("  Dense split â†’ %d sub-chunks", len(sub_chunks))
#     return sub_chunks


# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# #  SECTION 7  â€”  DEDUP & FILTER
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# #  SECTION 8  â€”  PARALLEL LLM EXTRACTION
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
#         prompt = PACKING_LIST_PAGE_PROMPT.format(context=context, text=text)
...
#         n = len(parsed.get("packages", []))
#         _plog("  [PAR] %s complete â€” %d items", label, n)
...
#     # fallback totals â€” sum from packages
#     if not totals_found.get("total_gross_weight_kg"):
#         s = sum(p.get("gross_weight_kg") or 0 for p in filtered if p.get("gross_weight_kg"))
#         if s > 0:
#             totals_found["total_gross_weight_kg"] = round(s, 3)
#     if not totals_found.get("total_net_weight_kg"):
...
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# #  SECTION 9  â€”  MERGE ALL CHUNKS  (last chunk only)
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
#     # â”€â”€ ALWAYS recompute totals from the full merged package list â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#     # Do NOT trust chunk-1's partial total_net_weight_kg
#     s = sum(p.get("gross_weight_kg") or 0 for p in filtered if p.get("gross_weight_kg"))
#     if s > 0:
#         totals_found["total_gross_weight_kg"] = round(s, 3)
...
#     s = sum(p.get("cbm") or 0 for p in filtered if p.get("cbm"))
#     if s > 0:
#         totals_found["total_cbm"] = round(s, 3)

#     # Only override if the document-level total is present (more accurate)
#     # i.e. if any chunk returned an explicit total, prefer that over sum
...
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# #  SECTION 10  â€”  OTM PAYLOAD BUILDER
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# #  SECTION 11  â€”  SELF-CHAIN: FIRE NEXT EXTRACTOR
# # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# def fire_next_extractor(fn_client, manifest_chunk: dict, request_id: str,
#                         total_chunks: int, order_base_gid: str, ob_ship_unit_gid: str,
#                         shipment_id: str = "", supplier_name: str = ""):   # â† ADD
#     # payload = {
...
# # def _check_and_fire_merger(os_client, shipment_id: str, request_id: str):
# #     if not shipment_id or not MERGER_FUNCTION_ID:
# #         log.info("Merger check skipped â€” no shipmentId or MERGER_FUNCTION_ID")
# #         return
...
# #     # â”€â”€ Phase 1: wait for manifest to exist â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# #     # The classifier writes the manifest after firing orchestrators, so there
# #     # is a small window where a fast extractor arrives before it exists.
# #     manifest_raw = None
# #     for wait_attempt in range(10):
# #         manifest_raw = download_from_os(os_client, manifest_key)
# #         if manifest_raw:
# #             break
# #         wait = 1.5 * (wait_attempt + 1)   # 1.5s, 3s, 4.5s â€¦ up to 15s
# #         _plog("Manifest not yet available â€” waiting %.0fs (attempt %d/10)", wait, wait_attempt + 1)
# #         time.sleep(wait)
...
# #     # â”€â”€ Phase 2: update-and-verify loop (handles concurrent write races) â”€â”€
# #     for attempt in range(5):
# #         manifest_raw = download_from_os(os_client, manifest_key)
# #         if not manifest_raw:
# #             log.warning("Shipment manifest disappeared at %s â€” retrying", manifest_key)
# #             time.sleep(1)
# #             continue
...
# #             if doc_info.get("requestId") == request_id:
# #                 if doc_info.get("status") == "complete":
# #                     already_marked = True
# #                     _plog("%s already marked complete â€” skipping write", doc_type)
# #                 else:
# #                     doc_info["status"] = "complete"
# #                     _plog("Marked %s as complete in shipment manifest", doc_type)
...
# #             verify_raw = download_from_os(os_client, manifest_key)
# #             if not verify_raw:
# #                 _plog("Verify download failed â€” retrying write (attempt %d)", attempt + 1)
# #                 continue

# #             verify_docs = json.loads(verify_raw).get("documents") or {}
# #             our_status  = next(
# #                 (d.get("status") for d in verify_docs.values()
# #                  if d.get("requestId") == request_id),
# #                 None,
# #             )
# #             if our_status != "complete":
# #                 _plog("Race detected on attempt %d â€” retrying write", attempt + 1)
# #                 continue
...
# #         else:
# #             time.sleep(0.3)
# #             latest_raw = download_from_os(os_client, manifest_key)
# #             if latest_raw:
# #                 documents = json.loads(latest_raw).get("documents") or {}

# #         total    = len(documents)
# #         complete = sum(1 for d in documents.values() if d.get("status") == "complete")
# #         _plog("Shipment %s â€” %d/%d docs complete", shipment_id, complete, total)

# #         if complete < total:
# #             _plog("Not all docs done yet â€” merger will be triggered by last extractor")
# #             return

# #         _plog("All %d docs complete â€” firing ai-shipment-merger (detached)", total)
# #         try:
# #             request_ids = {
# #                 doc_type: doc_info.get("requestId")
# #                 for doc_type, doc_info in documents.items()
# #                 if doc_info.get("requestId")
# #             }
# #             signer    = get_resource_principals_signer()
# #             fn_client = FunctionsInvokeClient(
# #                 config={}, signer=signer,
...
# #                 fn_invoke_type="detached",
# #             )
# #             _plog("Merger fired successfully for shipment %s", shipment_id)
# #         except Exception as exc:
# #             log.error("Failed to fire merger: %s", exc)
# #         return

# #     log.error("Failed to update manifest after 5 attempts â€” race condition unresolved")
...
# #         manifest_raw = download_from_os(os_client, manifest_key)
# #         if not manifest_raw:
# #             log.warning("Shipment manifest not found at %s â€” skipping", manifest_key)
# #             return
...
# #         already_marked = False                          # â† must be indented 8 spaces
# #         for doc_type, doc_info in documents.items():
# #             if doc_info.get("requestId") == request_id:
# #                 if doc_info.get("status") == "complete":
# #                     already_marked = True
...
# #                     _plog("Race detected on attempt %d â€” retrying write", attempt + 1)
# #                     continue
# #             documents = json.loads(verify_raw).get("documents") or {} if verify_raw else documents
# #         else:
# #             time.sleep(0.3)
# #             latest_raw = download_from_os(os_client, manifest_key)
# #             if latest_raw:
...
#     # â”€â”€ STEP 1: Extract this chunk â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#     try:
...
#     # # â”€â”€ STEP 2: Save chunk JSON â†’ Packing List/{req}/chunks/chunk_{N}.json
...
#         _check_and_fire_merger(os_client, shipment_id, request_id, "packing_list")
#         return fdk_response.Response(
#             ctx,
...
#     # â”€â”€ STEP 3: Append/overwrite job CSV â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
#     # â”€â”€ STEP 4: Count completed chunks â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
#     # â”€â”€ STEP 5: Self-chain OR merge â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
#         _plog("=" * 60)
#         _check_and_fire_merger(os_client, shipment_id or manifest.get("shipmentId", ""), request_id, "packing_list")  # âœ…
...
# ##the abvoe is working fine below are trials working with page wise llm call parallel for every 6 pages.
# """
# OTM Packing List Extractor  â€”  OCI FUNCTIONS VERSION  v5.0.0
# =============================================================
# Changes from v4:
...
# # - "packages" is a FLAT list of every individual PACKAGE (identified by pkg_no like "1 OF 5")
# # - pkg_no is the PACKAGE NUMBER like "1 OF 5", "2 OF 5" â€” NOT the item number
# # - Every item number shown in the packing list is a separate line item.
# # - Do NOT treat additional item numbers as accessories or sub-items unless the document explicitly labels them as ACCESSORY, SPARE PART, TRANSPORTATION SADDLE, BRACKET, or similar.
# # - Steel Structure item numbers such as 1320-2, 1320-4, 1135-27, 1155-31 are ALL line items and must be extracted individually.
# # - gross_weight_kg, dimensions_cm, cbm: from the MAIN package row only
# # - Only create sub_items when the document explicitly shows accessory descriptions without an item number.
# # - If a row has its own item number, it MUST become a separate line item. (e.g. "Transportation Saddle", saddles, brackets, accessories):
# #     - These are rows with NO pkg_no, NO quantity, NO gross_weight, NO dimensions
# #     - They belong to the PARENT package above them
# #     - Collect them into a "sub_items" array nested inside the parent package object
# #     - Do NOT create a separate top-level package entry for them
...
# # 1. Extract ALL item numbers first.
# # 2. Create one JSON record for EVERY item number.
# # 3. Verify that every item number appearing in the PDF appears in the JSON.
# # 4. Never skip intermediate item numbers.
# # 5. Count extracted line items before returning.
# # 6. If 114 item numbers appear in the PDF, JSON must contain 114 item records.
# # 7. Missing even one item number is considered extraction failure.
...
# # â”€â”€ SUPPLIER-SPECIFIC PL PROMPTS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
#             # â”€â”€ NEW: flatten line_items into all_packages â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
#             if line_items:
#                 # New structure â€” flatten each line_item
...
#             else:
#                 # Old/supplier-specific structure â€” use pkg directly
#                 if pkg_no:
#                     pkg["pkg_no"] = pkg_no
...
#     # fallback totals â€” sum from packages
#     # net_weight: ALWAYS recompute from flat items (independent of gross)
#     s = sum(p.get("net_weight_kg") or 0 for p in filtered if p.get("net_weight_kg"))
#     if s > 0:
#         totals_found["total_net_weight_kg"] = round(s, 3)

#     # gross_weight + cbm: count ONCE per pkg_no (only if LLM didn't return them)
#     if not totals_found.get("total_gross_weight_kg"):
#         seen_pkg  = set()
#         gross_sum = 0.0
#         cbm_sum   = 0.0
...
#     # net_weight: sum ALL flat items
#     s = sum(p.get("net_weight_kg") or 0 for p in filtered if p.get("net_weight_kg"))
#     if s > 0:
#         totals_found["total_net_weight_kg"] = round(s, 3)

#     # gross_weight + cbm: sum ONCE per pkg_no
#     seen_pkg  = set()
#     gross_sum = 0.0
#     cbm_sum   = 0.0
#     for p in filtered:
...
#     # prefer document-level totals from LLM if present
#     for field in ["total_gross_weight_kg", "total_net_weight_kg", "total_cbm",
#                   "total_packages", "total_quantity"]:
#         val = totals_found.get(field)
#         if val is not None:
...
#         doc_key = body.get("docKey") or "packing_list"
...
#############the above is working but the below is line item wise like per 20 line items 1 llm call.


 
 
##the abvoe is working fine below are trials
"""
OTM Packing List Extractor  â€”  OCI FUNCTIONS VERSION  v5.0.0
=============================================================
Changes from v4:
...
def _split_dense_page(page_text: str, max_rows: int) -> List[str]:
    ...
    # Broadened data-row detector - catches all 6 supplier formats
    ...
    while batch_start < len(body_lines):
        ...
        # Memory envelope - header into every sub-chunk (FIX 3)
        text = header + "\n"
 
        # Overlap context from previous batch (FIX 2)
        if batch_start > 0:
...
def merge_all_chunks(os_client, safe_req: str, total_chunks: int) -> dict:
    ...
    # FIX 4: ALWAYS use line-item-computed totals for weight/CBM.
    # Never trust the LLM's document-level summary fields â€” they can be
    # swapped (net > gross as seen in SupplierA) or copy quantity into net_weight.
    # Only use LLM totals for total_packages and total_quantity where
    # line-item computation isn't straightforward.
    for field in ["total_packages", "total_quantity"]:
        val = totals_found.get(field)
        if val is not None:
            base_result[field] = val
    # Weight and CBM: always use our computed values (never LLM summary)
    for field in ["total_gross_weight_kg", "total_net_weight_kg", "total_cbm"]:
        val = totals_found.get(field)
        if val is not None:
            base_result[field] = val
