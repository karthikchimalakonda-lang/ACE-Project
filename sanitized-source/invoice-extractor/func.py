...
# #     _plog("Invoice parallel extraction: %d page(s), %d workers", total_calls, actual_workers)

# #     def _call_one(args: Tuple[int, str]) -> Tuple[int, dict]:
# #         page_idx, text = args
# #         prompt = INVOICE_PAGE_PROMPT.format(text=text)
# #         log.info("  [PAR] Page %d firing", page_idx)
# #         try:
# #             raw    = call_cohere_with_retry(client, SYSTEM_PROMPT, prompt, MAX_TOKENS)
# #             parsed = parse_json_response(raw)
# #         except Exception as exc:
# #             log.error("  [PAR] Page %d FAILED: %s", page_idx, exc)
# #             parsed = {"line_items": [], "_error": str(exc)}
# #         _plog("  [PAR] Page %d complete â€” %d items", page_idx, len(parsed.get("line_items", [])))
...
# #                 log.error("  [PAR] Page %d future raised: %s", item[0], exc)
# #                 raw_results.append((item[0], {"line_items": []}))
...
# # # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# # #  SECTION 8  â€”  OTM PAYLOAD BUILDER FOR INVOICE
# # # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# #     # STEP 1: Extract this chunk in parallel
# #     try:
...
# #         _check_and_fire_merger(os_client, shipment_id, request_id, "invoice")
# #         return fdk_response.Response(
# #             ctx,
...
# #         _check_and_fire_merger(os_client, shipment_id or manifest.get("shipmentId", ""), request_id, "invoice")
...
# ###########################################################the above is working below are supplier wise trials
# """
# AI Invoice Extractor  â€”  OCI FUNCTIONS v1.0.0
# =============================================
# ROLE: Self-chaining parallel chunk processor for Commercial Invoices.
...
# # â”€â”€ SUPPLIER-SPECIFIC INVOICE PROMPTS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
#     return INVOICE_PAGE_PROMPT.format(text=text)  # SupplierA, TCL
...
# ###########################################################the above is working below are supplier wise trials and working the below completely but 4 supplier prompts
# """
# AI Invoice Extractor  â€”  OCI FUNCTIONS v1.0.0
# =============================================
# ROLE: Self-chaining parallel chunk processor for Commercial Invoices.
...
###########################################################the above is working below are supplier wise trials working with parallell6 page chunk
# """
# AI Invoice Extractor  â€”  OCI FUNCTIONS v1.0.0
# =============================================
# ROLE: Self-chaining parallel chunk processor for Commercial Invoices.
...
def extract_pages_parallel(
    ...
    # Build work items â€” applying dense-page splitting per page (Fix 1/2/3)
